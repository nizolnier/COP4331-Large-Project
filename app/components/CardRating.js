...
const CardRating = ({avgRating, numRatings}) => {
    ...
    return (
        <View className="flex flex-row items-center justify-center p-1">
            ...
            <Ionicons name="chatbubble-ellipses-outline" color="gray"/> 
            <Text onPress={onPress} className="pl-2 text-xs font-medium text-gray-400 underline hover:no-underline dark:text-white">{numRatings}</Text>
        </View>
    )
}
