...
function reduce(image, steps, angle) {
  ...
  resized.scan(0, 0, resized.bitmap.width, resized.bitmap.height, function (
    x,
    y,
    idx
  ) {
...
export default async function imageToGradient(imagePath, options, callback) {
  ...
  await Jimp.read(imagePath, function (err, image) {
    if (err) {
      callback(err, null);
      return;
    }
